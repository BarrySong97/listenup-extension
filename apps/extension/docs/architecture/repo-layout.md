...
>
> 源码路径: `apps/extension/src/`, `apps/extension/public/`, `apps/extension/manifest.json`, `apps/extension/vite.config.*`, `.github/workflows/`, `LICENSE`
>
> 覆盖范围: 目录职责、关键配置入口、哪些目录是当前主战场，哪些只是配套

## 源码定位

- 主路径: `apps/extension/src/`
- 相关路径: `apps/extension/public/`
- 相关路径: `apps/extension/manifest.json`
- 相关路径: `.github/workflows/ci.yml`

## 目录结构

以下路径默认相对 `apps/extension/`：
...
- `.github/workflows/ci.yml`: 仓库级 CI，从根目录使用 `pnpm install --frozen-lockfile` 和 `pnpm build:chrome` 构建 Chrome 扩展
