...
const DEFAULTS = {
  ...
  ignoreFiles: [],            // 相对仓库根的 posix 路径片段;命中的源文件跳过文件头检查(只用于工具生成、会被覆写的文件)
...
const isIgnoredFile = (abs) => {
  const rel = toPosix(relative(ROOT, abs));
  return (CONFIG.ignoreFiles ?? []).some((frag) => rel.includes(frag));
};

const sourceFiles = [];
for (const r of roots) {
  const base = join(ROOT, r);
  if (existsSync(base)) for (const f of walk(base)) if (hasSourceExt(f) && !isIgnoredFile(f)) sourceFiles.push(f);
}
